...
export function InvoiceDocument({ invoice }: { invoice: InvoiceData }) {
  ...
  const freelancerDisplay = invoice.user.name ?? invoice.user.email;
  const docAuthor = invoice.user.name ?? "Friday";

  return (
    <Document title={`Invoice #${shortId}`} author={docAuthor}>
      <Page size="A4" style={styles.page}>
        {/* Header */}
        <View style={styles.header}>
          {invoice.user.logoUrl ? (
            <Image src={invoice.user.logoUrl} style={styles.logoImage} />
          ) : (
            <Text style={styles.wordmark}>{freelancerDisplay}</Text>
          )}
        ...
        <View style={styles.footer} fixed>
          <Text style={styles.footerText}>
            {freelancerDisplay} · {invoice.user.email}
          </Text>
          <Text
