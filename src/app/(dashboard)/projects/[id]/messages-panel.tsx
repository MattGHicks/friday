...
import { sendFreelancerMessage } from "./message-actions";
import type {
  MessageAuthorType,
  SystemEventType,
...
function systemLabel(
  ...
  switch (type) {
    ...
    case "REVIEW_CHANGES_REQUESTED":
      return "Changes requested on design";
    case "INVOICE_SENT":
...
export function MessagesPanel({ projectId, messages }: Props) {
  ...
  const handleSend = () => {
    ...
    startTransition(async () => {
      const result = await sendFreelancerMessage(projectId, trimmed);
      if (result.error) {
      ...
      {messages.length === 0 ? (
        <div className="rounded-lg border border-dashed border-border/40 bg-card/40 p-6 text-center">
          <p className="text-sm text-muted-foreground">
            No messages yet. Send your client a note — they'll get it by email
            and can reply right from their inbox.
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div
                  className={`max-w-[85%] whitespace-pre-wrap rounded-xl border px-4 py-2.5 text-sm leading-relaxed ${
                    isUser
                      ? "border-fire/30 bg-fire/10 text-foreground"
